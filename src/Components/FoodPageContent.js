...
const FoodList = () => {
  ...
  const foodItems = [
    ...
    { id: 3, foodName: 'Bamboo shoots fry',Cuisine:'Cuisine : Assam' ,imageUrl: 'https://ohsnapletseat.com/wp-content/uploads/2021/12/IMG_1357-scaled-735x551.jpg' },
  ...
  const fastFoods = [
    ...
    { id: 12, Restaurant: "Taco Bell",imageUrl: 'https://upload.wikimedia.org/wikipedia/en/thumb/b/b3/Taco_Bell_2016.svg/1200px-Taco_Bell_2016.svg.png' },
    ...
    <div className='food-page-content-container' style={{ paddingLeft: '50px' }}>
      ...
      <Grid container spacing={3}>
        {foodItems.map((food, index) => (
          <FoodCard key={index} data={food} index={index} />
        ))}
      </Grid>
      ...
      <Grid container spacing={3}>
        {KoreanFoods.map((korean, index) => (
          <FoodCard key={index + KoreanFoods.length} data={korean} index={index + KoreanFoods.length} />
        ))}
      </Grid>
    </div>
  );
